...
/**
 * Considered as a debugging RenderContext. If rendered all known layer types if visible.
 * <p>
 * For a complete game you will probably assign the LayerRenderers by yourself.
 *
 * @author Andre Michelle
 */
public final class DebugScreen implements GLSurface.Renderer, RendererContext
{
	...
	private final List<Renderer> pipe;
	...
	public DebugScreen( @Nonnull final MapData mapData, final int width, final int height )
	{
								   ...
								   final boolean transparent )
	{
		...
		console.log( "calls: " + glRenderer.drawCalls, "images: " + glRenderer.drawImages );
	}
	...
	@Nonnull
	private List<Renderer> initPipe()
	{
		final List<Renderer> pipe = Lists.newArrayList();
		...
		for( final MapLayer mapLayer : mapData.mapLayers )
		{
			...
			{
				renderer = new TileRenderer( this, ( MapTileLayer ) mapLayer );
			}
			else if( MapObjectGroupLayer.class.equals( layerClass ) )
			{
				renderer = new ObjectsRenderer( this, ( MapObjectGroupLayer ) mapLayer );
			}
			else
			{
				renderer = null;
